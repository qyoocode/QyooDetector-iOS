...
void FeatureDotsProcessor::findDotsGradient()
{
	qyooModel qyooModel;
	...
	CannyGradientAndTheta(gaussImg,gradImg,NULL);
	...
	for (unsigned int row=0;row<numRow;row++)
	{
		...
		for (unsigned int pos=0;pos<numPos;pos++)
		{
			...
			// If we're in a gradient hole here, there must be a dot
			if (isInHole(gradImg,rowPix,posPix,PixelsPerDot,2))
				resChar |= 1<<pos;
		}
		
		unsigned char theChar;
		
		// JBB: Added following row
		qyooRows [numRow - row - 1] = resChar;
...
//	gaussFilter = MakeGaussianFilter(7,1.4);
	gaussImg = new RawImageGray8(grayImg->getSizeX(),grayImg->getSizeY());

	// Reduce noise with the Gauss filter
	gaussFilter->processImage(grayImg, gaussImg);
//	gdImagePtr grayOut = gaussImg->makeGDImage();
//      gdSaveAsPng(grayOut,"gray.png");
...
//	gdImagePtr testImg = gradImg->makeImage(true);
//	gdSaveAsPng(testImg,"grad.png");
...
#else
void FeatureProcessor::redoGradient(UIImage *inImage, int sizeX, int sizeY, bool flip, bool vertFlip)
#endif
{
	if (grayImg)
		delete grayImg;
...
{
	// Look for useful edges and form features
  //	featSet->sizeX = grayImg->getSizeX();  featSet->sizeY = grayImg->getSizeY();
    
	featImg = new RawImageGray32(grayImg->getSizeX(),grayImg->getSizeY());
	CannyFindFeatures(gradImg, thetaImg, 10.0, 60.0, feats, featImg);
// printf("Found %d potential features\n",feats.size());
