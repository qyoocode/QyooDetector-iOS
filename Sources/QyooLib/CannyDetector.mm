...
//			int g = sqrt((float)gx*gx+(float)gy*gy);
...
//				if (theta != 0.0 && gx != 0)
//					printf("gx = %d, gy = %d, theta = %f, thetaPix = %d\n",gx,gy,theta,thetaPix);
				}
...
//				printf("Feature (%d) = %d pixels\n",featId,featCount);
				
				// If it was a really small feature, nuke it
/*				if (featCount < FeatureThreshhold)
				{
					ScrubFeature(featImg,featId);
					feats.resize(feats.size()-1);
				} else */
					featId++;
			}
